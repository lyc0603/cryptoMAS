...
OUTPUT_DIR = Path("processed_data/benchmarks")
...
def train_lstm_models(train_end: str = BACKTEST_START) -> dict[str, tuple]:
    ...
    results = {}
    logger.info(
        "LSTM: training %d models on data before %s (device=%s)",
        len(UNIVERSE),
        train_end,
        LSTM_DEVICE,
    )
    ...
    for sym in UNIVERSE:
        ...
        for epoch in range(LSTM_EPOCHS):
            ...
            if epoch % 20 == 0:
                logger.debug(
                    "LSTM [%s] epoch %d — loss %.6f",
                    sym,
                    epoch,
                    loss_fn(model(X.to(LSTM_DEVICE)), Y.to(LSTM_DEVICE)).item(),
                )
...
def _build_nf_df(end_ts: pd.Timestamp) -> "pd.DataFrame":
    ...
    for sym in UNIVERSE:
        ...
        close.columns = ["ds", "y"]
        close["unique_id"] = sym
        close["ds"] = close["ds"].dt.tz_localize(
            None
        )  # neuralforecast expects tz-naive
        frames.append(close[["unique_id", "ds", "y"]])
    return pd.concat(frames, ignore_index=True)


def train_neural_model(
    model_name: str, train_end: str = BACKTEST_START
) -> "_NeuralForecast":
    """
    ...
    logger.info(
        "Neural [%s]: training on %d rows before %s …",
        model_name,
        len(train_df),
        train_end,
    )
...
def _tune_params(strategy: str) -> dict:
    ...
    logger.info(
        "Tuning [%s] on %s → %s (%d combos)", strategy, VAL_START, VAL_END, len(grid)
    )
    for params in grid:
    ...
    logger.info(
        "Tuning [%s] best=%s (val_return=%+.4f)", strategy, best_params, best_ret
    )
    return best_params
...
class Portfolio:
    ...
    def apply_actions(self, actions: list[dict], prices: dict[str, float]) -> None:
        ...
        desired = {item["symbol"]: frac * post_sell_cash for item, frac in buy_items}
...
def get_signals(as_of: pd.Timestamp, strategy: str) -> list[str]:
    ...
    for sym in UNIVERSE:
        ...
        if strategy == "sma7":
            ...
            if (
                pd.notna(row.get("sma_7"))
                and pd.notna(row.get("sma_30"))
...
def compute_actions(
    ...
    """
    is_uninitialised = (
        not portfolio.holdings and abs(portfolio.cash - portfolio.initial_value) < 0.01
    )
            ...
            actions.append(
                {
                    ...
                    "action": -0.5,
                    "rationale": (
                        "Sell: position exiting signal set"
                        if sym not in signal_set
...
def run_strategy(strategy: str, weeks: list[str]) -> None:
    ...
    logger.info("=" * 60)
    logger.info(
        "Benchmark: %s  |  weeks: %d  |  out: %s", combo_name, len(weeks), out_dir
    )
    logger.info("=" * 60)
    ...
    if not weeks_todo:
        logger.info(
            "[%s] All %d weeks already computed — skipping.", combo_name, len(weeks)
        )
        return
    ...
    for week in weeks:
        ...
        logger.debug(
            "[%s] %s — signals: %s", combo_name, week, signals if signals else "(none)"
        )

        mcap_weights = get_mcap_weights(sunday) if strategy == "mcap_hold" else None
        actions = compute_actions(
            strategy, portfolio, signals, mcap_weights=mcap_weights
        )
